...
def getAccuracyPlotNew(MT):
    ...
    for Algo in [CNN, NB, NN, KNN, SVM]:
        ...
        accMatrix = np.array(accMatrix)
        if (Algo == KNN or Algo==SVM):
            yMatrix = np.reshape(yMatrix, (1, yMatrix.shape[0], 20800))
            predMatrix = np.reshape(predMatrix, (1, predMatrix.shape[0], 20800))
            accMatrix = np.reshape(accMatrix, (1, accMatrix.shape[0], 26,26))
...
#                 accMatrix[x] = accMatrix[x][22:78]
    ...
    plt.suptitle('MT: '+str(MT), fontsize=14, fontweight='bold')
